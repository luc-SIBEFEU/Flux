...
@section('titre_page', __('sidebar.rapports_financiers'))
@section('titre', __('sidebar.rapports_financiers') . ' — ' . __('sidebar.espace_admin'))

@section('contenu')

<div class="bg-white border border-black/5 rounded-2xl p-6 mb-6">
    <p class="text-xs text-flux-noir/40 uppercase tracking-wide">{{ __('admin_rapports.revenu_total') }}</p>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="lg:col-span-2 bg-white rounded-2xl border border-black/5 p-6">
        <h3 class="font-medium mb-1">{{ __('admin_rapports.revenus_par_hotel') }}</h3>
        <p class="text-xs text-flux-noir/40 mb-4">{{ __('admin_rapports.top_hotels_ca') }}</p>
    ...
    <div class="bg-white rounded-2xl border border-black/5 p-6">
        <h3 class="font-medium mb-1">{{ __('admin_rapports.repartition_ville') }}</h3>
        <p class="text-xs text-flux-noir/40 mb-4">{{ __('admin_rapports.part_revenus') }}</p>
