...
@section('titre_page', __('sidebar.demandes_baye'))
@section('titre', __('sidebar.demandes_baye') . ' — ' . __('sidebar.espace_bailleur'))

@section('contenu')

<div class="flex gap-2 mb-6 overflow-x-auto carte-scroll">
    @foreach(['' => __('reservation.tab_toutes'), 'nouveau'=>__('demande.tab_nouvelles'), 'validee'=>__('demande.tab_validees'), 'rejetee'=>__('demande.tab_rejetees')] as $val=>$label)
        ...
        <div class="bg-white border border-black/10 rounded-2xl p-5">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-flux-noir/50 mt-1">{{ __('logement.type_' . $demande->logement->type) }} — {{ $demande->logement->quartier }}, {{ $demande->logement->ville }}</p>
                    @if($demande->message)
                ...
                @else
                    ...
                    <span class="text-xs px-2.5 py-1 rounded-full font-medium shrink-0 {{ $badges[$demande->statut] }}">{{ __('demande.statut_' . $demande->statut) }}</span>
                @endif
            </div>
        </div>
    @empty
