...
@section('titre_page', __('sidebar.mes_reservations'))
@section('titre', __('sidebar.mes_reservations') . ' — Flux')

@section('contenu')

<div class="flex gap-2 mb-6 overflow-x-auto carte-scroll">
    @foreach(['tout'=>__('common.tout'),'en_attente'=>__('reservation.tab_en_attente'),'confirmee'=>__('reservation.tab_confirmees'),'annulee'=>__('reservation.tab_annulees')] as $val=>$label)
...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
        ...
        <div class="bg-white border border-black/10 rounded-2xl overflow-hidden">
            ...
            <div class="p-5">
                <div class="flex items-center justify-between mb-2">
                    ...
                    @endphp
                    <span class="text-xs px-2.5 py-1 rounded-full font-medium {{ $badges[$reservation->statut] ?? '' }}">{{ __('reservation.statut_' . $reservation->statut) }}</span>
                </div>
                ...
                <span class="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full font-medium mt-2 {{ $badgesPaiement[$reservation->statut_paiement] ?? '' }}">
                    {{ __('reservation.paiement_label') }} : {{ __('reservation.statut_paiement_' . $reservation->statut_paiement) }}
                </span>
    ...
    @empty
        <div class="col-span-full text-center py-16 text-flux-noir/40">
            ...
            {{ __('reservation.aucune_dans_categorie') }}
        </div>
    @endforelse
</div>
