@extends('layouts.app')
@section('titre', ucfirst($logement->type) . ' à ' . $logement->ville . ' — Flux')

@section('contenu')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">

    <div class="mb-6">
        <span class="text-xs font-semibold bg-flux-violet-pale text-flux-violet px-3 py-1 rounded-full capitalize">{{ $logement->type }} · {{ $logement->categorie === 'meuble' ? 'Meublé' : 'Standard' }}</span>
        <h1 class="font-display text-3xl sm:text-4xl text-flux-noir mt-3">{{ ucfirst($logement->type) }} — {{ $logement->quartier }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div class="lg:col-span-2 space-y-10">
            @if($logement->info)
                <section>
                    <h2 class="font-display text-2xl mb-3">Description</h2>
            ...
            @if($logement->equipements->isNotEmpty())
                <section>
                    <h2 class="font-display text-2xl mb-3">Équipements</h2>
            ...
            <section>
                <h2 class="font-display text-2xl mb-4">Commentaires</h2>
                <div class="space-y-4">
                    ...
                    @empty
                        <p class="text-sm text-flux-noir/40">Aucun commentaire pour le moment.</p>
                    @endforelse
                </div>
                ...
                @auth
                    <form action="{{ route('commentaires.store', $logement) }}" method="POST" class="mt-6 bg-white border border-black/10 rounded-2xl p-5">
                        @csrf
                        <label class="text-sm font-medium">Laisser un commentaire</label>
                        ...
                        <div class="flex items-center gap-3 mt-3">
                            ...
                            <button class="bg-flux-violet text-white text-sm font-medium px-4 py-2 rounded-lg">Publier</button>
                        </div>
                    </form>
                @endauth
            </section>
