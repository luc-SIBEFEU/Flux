...
@section('content')
<div class="p-6">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Galerie photo — {{ $chambre->nom }}</h1>

    <form method="POST" action="{{ route('hotelier.rooms.gallery.store', $chambre) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow border border-gray-100 p-5 mb-6">
        @csrf
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="relative group">
            ...
            <form method="POST" action="{{ route('hotelier.rooms.gallery.destroy', [$chambre, $image]) }}" onsubmit="return confirm('Supprimer cette image ?');" class="absolute top-1 right-1">
                @csrf
                @method('DELETE')
